...
do
    sbatch << SLURM
#!/bin/bash
#SBATCH --job-name=syco_en_m-1.5_l${LAYER}
#SBATCH --output=/users/acp25mk/cross-lingual-caa/logs/syco_en_m-1.5_layer${LAYER}_%j.out
#SBATCH --error=/users/acp25mk/cross-lingual-caa/logs/syco_en_m-1.5_layer${LAYER}_%j.err
...
cfg.generation_output_dir = "/users/acp25mk/cross-lingual-caa/results/syco_en_m-1.5_layer${LAYER}"

vector_applier = BaseVectorApplier(cfg)
vector_applier.apply_vectors()

datasets = {
    "syco_en_m-1.5_layer${LAYER}": json.load(open("/users/acp25mk/cross-lingual-caa/data/sycophancy_en_train.json"))
}
vector_applier.generate(datasets, save_results=True)
print("Done.")
PY
SLURM
    echo "Submitted syco_en_m-1.5 layer ${LAYER}"
done
echo "All EN syco m-1.5 submitted!"
