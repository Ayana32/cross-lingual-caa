#!/bin/bash
for LANG in ko es kk it zh; do
for LAYER in 12 13 14 15 16 17 18; do

    # IT/ZH는 1.3B 파일명
    if [ "$LANG" = "it" ] || [ "$LANG" = "zh" ]; then
        DATAFILE="/users/acp25mk/cross-lingual-caa/data/sycophancy_${LANG}_train_1.3B_matched.json"
    else
        DATAFILE="/users/acp25mk/cross-lingual-caa/data/sycophancy_${LANG}_train_matched.json"
    fi
