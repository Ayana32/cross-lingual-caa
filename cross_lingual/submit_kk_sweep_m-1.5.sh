...
do
    sbatch << SLURM
#!/bin/bash
#SBATCH --job-name=kk_m-1.5_l${LAYER}
#SBATCH --output=/users/acp25mk/cross-lingual-caa/logs/kk_m-1.5_layer${LAYER}_%j.out
#SBATCH --error=/users/acp25mk/cross-lingual-caa/logs/kk_m-1.5_layer${LAYER}_%j.err
...
cfg.generation_output_dir = "/users/acp25mk/cross-lingual-caa/results/semeval_kk_m-1.5_layer${LAYER}"

vector_applier = BaseVectorApplier(cfg)
vector_applier.apply_vectors()

datasets = {
    "semeval_kk_m-1.5_layer${LAYER}": json.load(open("/users/acp25mk/cross-lingual-caa/data/semeval/semeval_kk_train.json"))
}
vector_applier.generate(datasets, save_results=True)
print("Done.")
PY
SLURM
    echo "Submitted kk_m-1.5 layer ${LAYER}"
done
echo "All KK semeval m-1.5 submitted!"
