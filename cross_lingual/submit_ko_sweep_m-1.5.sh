...
do
    sbatch << SLURM
#!/bin/bash
#SBATCH --job-name=ko_m-1.5_l${LAYER}
#SBATCH --output=/users/acp25mk/cross-lingual-caa/logs/ko_m-1.5_layer${LAYER}_%j.out
#SBATCH --error=/users/acp25mk/cross-lingual-caa/logs/ko_m-1.5_layer${LAYER}_%j.err
...
cfg.generation_output_dir = "/users/acp25mk/cross-lingual-caa/results/semeval_ko_m-1.5_layer${LAYER}"

vector_applier = BaseVectorApplier(cfg)
vector_applier.apply_vectors()

datasets = {
    "semeval_ko_m-1.5_layer${LAYER}": json.load(open("/users/acp25mk/cross-lingual-caa/data/semeval/semeval_ko_train.json"))
}
...
SLURM
    ...
    echo "Submitted ko m-1.5 layer ${LAYER}"
done
echo "All KO m-1.5 layers submitted!"
