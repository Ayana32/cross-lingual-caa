...
Qwen3.5-9B: EN / IT / KK / ZH (Layers 12-18)
"""
...
VECTOR_DIRS = {
    "en": Path("/users/acp25mk/cross-lingual-caa/vectors/Qwen3.5-9B/sycophancy/caa_vector_multiple_choice"),
    "it": Path("/users/acp25mk/cross-lingual-caa/vectors/Qwen3.5-9B-it/sycophancy_it/caa_vector_multiple_choice"),
    "kk": Path("/users/acp25mk/cross-lingual-caa/vectors/Qwen3.5-9B-kk/sycophancy_kk/caa_vector_multiple_choice"),
    "zh": Path("/users/acp25mk/cross-lingual-caa/vectors/Qwen3.5-9B-zh/sycophancy_zh/caa_vector_multiple_choice"),
    "ko": Path("/users/acp25mk/cross-lingual-caa/vectors/Qwen3.5-9B-ko/sycophancy_ko/caa_vector_multiple_choice"),
}

OUTPUT_DIR = Path("/users/acp25mk/cross-lingual-caa/results/geometry_qwen35")
...
def run_global_pca(vecs):
    ...
    plt.tight_layout()
    out = OUTPUT_DIR / "global_pca_qwen35.png"
...
def plot_cosine(df_cos):
    ...
    plt.tight_layout()
    out = OUTPUT_DIR / "cosine_similarity_qwen35.png"
    plt.savefig(out, dpi=FIG_DPI)
    print(f"  Saved: {out}")
    plt.close()
...
if __name__ == "__main__":
    ...
    # Save results
    df_norm.to_csv(OUTPUT_DIR / "norm_analysis_qwen35.csv", index=False)
    df_cos.to_csv(OUTPUT_DIR / "cosine_analysis_qwen35.csv", index=False)
