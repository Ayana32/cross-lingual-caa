...
Panel B: Within-language layer-wise (L24-28, with Spearman ρ per language)
"""
import json
import numpy as np
import pandas as pd
...
COSINE_CSV  = Path("results/geometry_gemma4/cosine_analysis_gemma4.csv")
OUTPUT_DIR  = Path("results/geometry_gemma4")
LAYERS      = list(range(24, 29))

LANGS = ["it", "zh", "ko", "kk"]
PEAK_LAYERS = {"it": 26, "zh": 26, "ko": 26, "kk": 26}

RESULT_DIRS = {
    "it": "gemma4_syco_it_m-1.0_layer{L}",
    "zh": "gemma4_syco_zh_m-1.0_layer{L}",
    "ko": "gemma4_syco_ko_m-1.0_layer{L}",
    "kk": "gemma4_syco_kk_m-1.0_layer{L}",
}
...
def panel_a(ax, df):
    ...
    ax.set_xlabel("EN–target cosine similarity at L26", fontsize=10)
...
def panel_b(ax, df):
    ...
    ax.set_ylabel("|ΔSR| (pp)", fontsize=10)
    ax.set_title("B. Within-language, layer-wise (L24–28)", fontsize=11, fontweight="bold")
    ax.grid(True, alpha=0.3)
...
plt.suptitle("Relationship between Cross-lingual Vector Similarity and Steering Effect\n(Gemma4-12B-IT, Sycophancy)",
             fontsize=11, fontweight="bold", y=1.02)
plt.tight_layout()

out = OUTPUT_DIR / "cosine_delta_scatter_gemma4.png"
plt.savefig(out, dpi=150, bbox_inches="tight")
plt.close()
print(f"\nSaved: {out}")

# ── Spearman table ────────────────────────────────────────────
print("\nWithin-language Spearman ρ (descriptive, n=5):")
for lang, rho in rho_table.items():
    print(f"  {LANG_NAMES[lang]}: ρ = {rho:.3f}")

# Save data
df.to_csv(OUTPUT_DIR / "cosine_delta_gemma4.csv", index=False)
print(f"Saved: {OUTPUT_DIR}/cosine_delta_gemma4.csv")
