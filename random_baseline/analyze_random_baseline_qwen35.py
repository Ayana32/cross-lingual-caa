...
RESULTS_DIR = Path("results/random_baseline/qwen35_syco_en_layer15")
SOURCE_FILES = {
...
REAL_DELTAS = {"en": -18.5, "it": -21.1, "zh": -24.9, "ko": -19.1, "kk": -19.3}
...
def compute_delta(result_path, source_path):
    ...
    total = len(results)
    orig_correct, orig_unparse = 0, 0
    steer_correct, steer_unparse = 0, 0
    for idx, res in enumerate(results):
        ...
        if orig_pred is None or steer_pred is None:
            orig_unparse += int(orig_pred is None)
            steer_unparse += int(steer_pred is None)
            continue
        if orig_pred == gold_label: orig_correct += 1
        if steer_pred == gold_label: steer_correct += 1
    paired = sum(1 for i, res in enumerate(results)
                 if parse_prediction((res.get("orig_pred","") if not isinstance(res.get("orig_pred",""), list) else res.get("orig_pred","")[0])) is not None
                 and parse_prediction((res.get("pred","") if not isinstance(res.get("pred",""), list) else res.get("pred","")[0])) is not None)
    if paired == 0: return None
    orig_rate = orig_correct / paired * 100
    steer_rate = steer_correct / paired * 100
    return steer_rate - orig_rate
